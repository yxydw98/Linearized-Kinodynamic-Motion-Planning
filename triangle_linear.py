...
triangle_vertices = [(-1, -1, 0), (1, -1, 0), (0, 1, 0)]
...
triangle_collision_shape = p.createCollisionShape(shapeType=p.GEOM_MESH,
                                                  vertices=triangle_vertices,
                                                  meshScale=[1, 1, 1])
triangle_visual_shape = p.createVisualShape(shapeType=p.GEOM_MESH,
                                            vertices=triangle_vertices,
                                            meshScale=[1, 1, 1],
                                            rgbaColor=[1, 0, 0, 1])
disk2Id = p.createMultiBody(baseMass=1,
                                  baseCollisionShapeIndex=triangle_collision_shape,
                                  baseVisualShapeIndex=triangle_visual_shape,
                                  basePosition=[0, 0, 1],  # Position where the body will be created
...
while (len(dataset) < bufferSize):
    ...
    print("start_orientation", object_ori)
